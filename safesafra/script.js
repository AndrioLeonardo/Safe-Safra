...
//---API do clima---
    ...
    const fertilizanteOption = tipoRegistroSelect.querySelector('option[value="fertilizacao"]');
    const sintomaOption = tipoRegistroSelect.querySelector('option[value="sintoma"]');
    const pragaOption = tipoRegistroSelect.querySelector('option[value="praga"]');
    ...
    if (jaExistePlantio) {
        ...
        // Habilita as outras opções E garante que o texto delas volte ao normal
        fertilizanteOption.disabled = false;
        fertilizanteOption.textContent = 'Aplicação de Fertilizante';
        sintomaOption.disabled = false;
        sintomaOption.textContent = 'Sintoma de Doença';
        pragaOption.disabled = false;
        pragaOption.textContent = 'Presença de Praga';
    } else {
        ...
        // Desabilita as outras opções E adiciona o texto explicativo
        fertilizanteOption.disabled = true;
        fertilizanteOption.textContent = 'Fertilizante (requer plantio)';
        sintomaOption.disabled = true;
        sintomaOption.textContent = 'Sintoma (requer plantio)';
        pragaOption.disabled = true;
        pragaOption.textContent = 'Praga (requer plantio)';
    ...
    const secaoDiagnostico = document.getElementById('secao-diagnostico');
    const secaoPlantio = document.getElementById('secao-plantio');

    // Verificação de segurança: só tenta esconder um elemento se ele foi encontrado
    if (secaoDiagnostico) {
    ...
    // Mostra a seção correta, também verificando se o elemento existe antes de tentar mostrá-lo
    if (tipoRegistroSelect.value === 'sintoma' && secaoDiagnostico) {
        secaoDiagnostico.style.display = 'block';
    } else if (tipoRegistroSelect.value === 'plantio' && secaoPlantio) {
